...
<body>
	...
	<div class="pd-20">
		...
		<table class="table table-border table-bordered table-hover table-bg">
			...
			<tbody>
				<c:forEach items="${teachers }" var="teacherss">
					<tr class="text-c">
						<td id="tId">${teacherss.tId }</td>
						<td>${teacherss.tUsername }</td>
						<td>${teacherss.tPassword }</td>
						<td><a>${teacherss.tInsid }</a></td>
						<td><fmt:formatDate value="${teacherss.tCreateTime}"
								pattern="yyyy-MM-dd" /></td>
						<td class="f-14"><a title="编辑" href="javascript:;"
							onclick="admin_role_edit('修改教师信息','teacher/updateTeacherUi?tId=${teacherss.tId}','1')"
							style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a>
							<a title="删除" href="javascript:;"
							onclick="admin_role_del(this,'${teacherss.tId}')" class="ml-5"
